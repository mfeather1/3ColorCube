...
function solver_search(epn, etn, cpn, ctn, cp6cn, eprn, n, mvlist)
{
  for (var i=0, mv=0; (mv=mvlist[i]) != -1; i++) {
    ...
    if (n == depth) {
      ...
      var ept_op_ix = ept_op_idx[ep];
      if (ept_op_ix != -1) 
        var op = ept_min_op[ept_op_ix*2048+et];
      else
        op = ep_min_op[ep];
      var epmin = ep_min[ep];
      ...
      if (depth-n <= 11 && depth-n >= 9) {
        ...
        // p01 = 70*2187*782*16  16 = (8 of 11 et bits = 256) / 4 configs/byte / 4 parts
        // p02 = p01*2, p03 = p01*3
        const p01 = 1915462080, p02 = 3830924160, p03 = 5746386240;
        tmp = (ix < p02) ? 
          ((ix < p01) ? dist400[ix]     : dist401[ix-p01]) :
          ((ix < p03) ? dist402[ix-p02] : dist403[ix-p03]);
        tmp = (tmp>>((rs&3)<<1))&3;
        dist = (tmp) ? tmp+9+n : 0;
        if (dist > depth)
          continue;
      ...
      var epr = new Uint8Array(3);
      get_epr_mov2(epr, epn, eprn, mv);
      if (minmv-n <= 11 && minmv-n >= 9) {
        if (ept_op_ix != -1) {
          op = ept_op;
          if (ix2 != -1)
            op = get_min_op_e6c(ep, epr, op, ix2);
        }
        var eprsym = new Uint8Array(3);
        get_eprsym(eprsym, ep, epr, op);
        var eprsymn = eprsum(eprsym);
        var rs = etsym>>3;
        ix = (epmin*13824 + eprsymn)*64 + (rs>>2);
        var tmp = ((dist7[ix]>>((rs&3)<<1))&3);
        dist = (tmp) ? tmp+8+n : 0;
        // nodes++;
        if (dist >= minmv) {
          // pruned++;
