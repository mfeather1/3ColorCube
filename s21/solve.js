...
var gdone;
var conc;
var get_etsym, get_ctsym, get_eprsym;
var search_time;
var first_time = 1;
var solution = [];
...
// shared arrays
var dist1, dist2, dist3, dist5, dist7, dist9, distp2;
...
var epr_mov2, epr_mov_idx;
var epr_sym, epr_sym2, epr_idx;
var min_ep, epr_min_op;
...
importScripts('../solver_qc/rch.js');
importScripts('../rclib.js');
importScripts('epr_sym.js');
...
function ipc(e) {
  ...
  if (cmd == 'init') {
    ...
    epr_idx = new Uint8Array(e.data.epr_idx);
    ...
    epr_mov2 = new Uint8Array(e.data.epr_mov2);
    epr_mov_idx = new Uint8Array(e.data.epr_mov_idx);
    min_ep = new Uint16Array(e.data.min_ep);
    epr_min_op = new Uint8Array(e.data.epr_min_op);
    dist_files_loaded = e.data.dist_files_loaded;
    dist_gen_depth = e.data.dist_gen_depth;
    conc = e.data.conc;
    use_p2seq = e.data.use_p2seq;
    if (EPR_SYM_METHOD == 1)
      epr_sym = new Uint8Array(e.data.epr_sym);
    else {
      epr_sym2 = new Uint8Array(e.data.epr_sym2);
      epr_idx = new Uint8Array(e.data.epr_idx);
    }
    if (USE_DIST5)
      dist5 = new Uint8Array(e.data.dist5);
    if (USE_DIST7)
      dist7 = new Uint8Array(e.data.dist7);
    if (CT_SYM_METHOD == 1)
  ...
  if (cmd == 'solve') {
    ...
    if (e.data.worker == 1 && gdone[0] == 0) {
      var msg = 'Searching &nbsp;';
      msg += ' <button class="btn svw1" style="margin-bottom:12px"';
      msg += ' onclick=stop_search()>Stop</button>';
      postMessage({'cmd': 'msg', 'msg': msg})
    }
    */
...
function solve_cube(facelets)
{
  ...
  if (ep == 0 && et == 0 && cp == 0 && ct == 0) {
    ...
    if (stoplen)
      ...
      minmv = presolved_dist + 1;
    if (worker == 2) {
      var s = 'Solvable in ' + presolved_dist + ' moves<br>';
      postMessage({'cmd': 'msg', 'msg': s});
    }
  }
  ...
  /*
  if (nodes > 0)
    console.log(worker, nodes, pruned, (pruned/nodes).toFixed(2));
  else
    console.log(worker, nodes, pruned, 0);
  */
}
...
function populate_distp2()
{
  ...
  if (use_p2seq) {
    var p2save = distp2
    distp2 = new Uint8Array(C_PRMr*E_PRMr);  // 8 MB
  }
  distp2[0] = 1;
...
{ /*
    ...
    Dist9 has the same components as DistP2 but differs in that it applies
    to all cube configs whereas DistP2 only applies to the relatively small
    subset of configs that are already solved as 3-color.
...
function get_min_op_e6c(ep, epr, op, ix) {	
