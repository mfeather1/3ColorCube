...
function AnimCube3(params) {
  ...
  function init() {
    ...
    if (demo)
      startAnimation(-1);
    if (typeof callback_acjs != 'undefined')
      callback_acjs(parNode.id);
  } // init()
  ...
  */
      ...
      for (var i = 0; i < 6; i++) { // all faces
        ...
        if (vProd(perspEye, faceNormals[i]) < -(1-scale)) { // draw only hidden faces
		  ...
		  /* commenting out the code that draws the facelet background
                  ...
                  */
                  if (fourColorCube)
  		    splitColor(graphics, fillX, fillY, i, scube[i][p * 3 + q], cube[i][p * 3 + q]);
                  else
  ...
  function splitColor(g, x, y, face, superTwist, color) {
    ...
    var c1 = colors[splitColors[color-1][0]];
    var c2 = colors[splitColors[color-1][1]];
    var dir = (color == 3 || color == 4) ? 1 : 0;
    g.beginPath()
    g.moveTo(x[c], y[c]);
    if (dir == 0) {
      g.lineTo(x[d], y[d]); 
        ...
        if (spin) {
          if (moveAnimated) {
            if (currentAngle * phis < phit) {
              // wait 1 millisecond before drawing first
              // frame of each move for smoother animation
              while (Date.now() - time0 < 1)
                ;  // empty loop
              paint();
