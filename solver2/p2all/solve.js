...
function ipc(e) {
  ...
  if (cmd == 'init') {
    ...
    if (use_p2seq == 1) {
      ...
      p2seq2 = new Int8Array(e.data.p2seq2);
      rr_min = new Int32Array(e.data.rr_min);
      rr_min_op = new Int8Array(e.data.rr_min_op);
    }
    var time0 = Date.now(); 
    init();
    load_search_code();
...
function solve_cube(facelets)
{
  ...
  if (ep == 0 && et == 0 && cp == 0 && ct == 0) {
    ...
    if (use_p2seq) {
      if (worker == 1)
        show_solution_from_p2seq2(cpr, eprn);
      return;
    }
    if (stoplen)
      minmv = stoplen + 1;
...
*/
function show_solution_from_p2seq2(cpr, epr) {
  var ix = cpr*6912 + Math.floor(epr/2);
  var min = rr_min[ix];
  var mop = rr_min_op[ix];
  for (var i=min*16, j=0; j < 16 && p2seq2[i+j] != 25; j++) {
    var v = op_mv[mop*18 + p2seq2[i+j]];
    solution[j] = disp[Math.floor(v/3)] + disp2[v%3];
  }
  sol_dep1 = 0;
  sol_dep2 = solution.length;
...
*/
function get_p2_seq2(cpr, epr) {
  var ix = cpr*6912 + Math.floor(epr/2);
  var min = rr_min[ix];
  var mop = rr_min_op[ix];
  for (var i=min*16, j=0; j < 16 && p2seq2[i+j] != 25; j++)
    seq2[j+1] = op_mv[mop*18 + p2seq2[i+j]];
  show_moves();
}
...
// ----------------------------------------------------------------------------
/*
function populate_distp2(n)
{
  console.log('populate_distp2:', n);
  var epr = new Uint8Array(3);
  var cp = 0;
  var ct = 0;
  var ep = 0;
